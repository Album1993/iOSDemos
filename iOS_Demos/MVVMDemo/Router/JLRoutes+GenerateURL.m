...
+ (NSString *)svw_generateURLWithPattern:(NSString *)pattern parameters:(NSArray *)parameters {
    return [[self class] svw_generateURLWithPattern:pattern parameters:parameters extraParameters:nil];
}

+ (NSString *)svw_generateURLWithPattern:(NSString *)pattern parameters:(NSArray *)parameters extraParameters:(NSDictionary *)extraParameters {
    // 创建路由
    ...
    if (routeValues && routeValues.count) {
        ...
        return [NSString stringWithFormat:@"/%@%@", urlPath, [self svw_mapDictionaryToURLQueryString:extraParameters]];
    }
    return nil;
}

+ (NSDictionary *)svw_parseParamsWithURL:(NSURL *)URL {
...
+ (NSString *)svw_mapDictionaryToURLQueryString:(NSDictionary *)dic {
    if (!dic) {
        return @"";
    }
